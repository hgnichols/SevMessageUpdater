...
/**
 * Controls everything connecting the DB to the client instantiated by a singleton
 * 
 * @author Thomas Polk (with many added methods by Hunter Nichols)
 *
 */
public class DatabaseController implements DatabaseConstants {
	...
	private DatabaseController() {
		/**
		 * This constructor is defined as private so no one is able to create a
		 * new instance of this class. The static method getDBController()
		 * should be used instead. This will ensure all classes share the same
	...
	public String buildConnectionString() {
		/**
		 * Builds a connection string from the component pieces provided in this
		 * class.
		 */
		StringBuffer buf = new StringBuffer();
	...
	public ArrayList<String> getDatabaseNames() {
		/**
		 * This method gets a list of all the databases on the server owned by
		 * dbo
		 */
	...
	public ArrayList<String> getTableNames() {
		/**
		 * This method gets a list of all tables in the currently active
		 * database
		 */
	...
	public void printResultSet(ResultSet rs) {
		/**
		 * This method just prints the contents of a ResultSet to the output
		 * console.
		 */
		try {
	...
	public void printQueryResults(String query) {
		/**
		 * This method attempts to execute the query string passed in and print
	...
	public ResultSet executeQuery(String query) {
		// Executes the query passed in on the active database and returns a
		// ResultSet
		Statement s = null;
		ResultSet rs = null;
		try {
	...
	public ResultSet getSalesPersonByID(String salesPersonID) {
		/**
		 * Returns a ResultSet containing at most one row of data about a sales
		 * person.
		 */
	...
	public ResultSet executeStoredProcedure(String procName, ArrayList<NameValuePair> nvpList) {
		/**
		 * Calls the stored procedure passed in, sending it the parameters
		 * passed in. Returns the resulting ResultSet
		 */
		ResultSet rs = null;
		StringBuilder buf = new StringBuilder();
		buf.append("{ call ");
	...
	public void executeCUDStoredProcedure(String procName, ArrayList<NameValuePair> nvpList) {
		/**
		 * Calls the stored procedure passed in, sending it the parameters
		 * passed in. Should only be used for stored procs that create, update,
		 * or delete records
		 */
		StringBuilder buf = new StringBuilder();
		buf.append("{ call ");
	...
	public ResultSet getCarByID(String carID) {
		/**
		 * Returns a ResultSet containing at most one row of data about a car
		 */
		ResultSet rs = null;
	...
	public ResultSet getCarsByPriceRange(int lowVal, int highVal) {
		/**
		 * Returns a ResultSet containing at all cars with a cost between the
		 * low and high values passed in.
		 */
		ResultSet rs = null;
		ArrayList<NameValuePair> nvps = new ArrayList<>();
	...
	public void updateUpdate(Update u) {
		...
		nvpList.add(
				new NameValuePair(DatabaseConstants.COLUMN_UPDATES_SEND_MESSAGE, Integer.toString(u.getSendMessage())));
		nvpList.add(new NameValuePair(DatabaseConstants.COLUMN_UPDATES_UPDATE_MESSAGES,
				Integer.toString(u.getUpdateMessages())));
		nvpList.add(new NameValuePair(DatabaseConstants.COLUMN_UPDATES_HEADING_UPDATE,
				Integer.toString(u.getHeadingUpdate())));
